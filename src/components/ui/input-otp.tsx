...
// New components for PIN entry
const PinInput = React.forwardRef<
  React.ElementRef<typeof OTPInput>,
  React.ComponentPropsWithoutRef<typeof OTPInput>
>(({ maxLength = 4, ...props }, ref) => (
  <InputOTP maxLength={maxLength} ref={ref} {...props} />
))
PinInput.displayName = "PinInput"

const PinInputGroup = InputOTPGroup

const PinInputSlot = React.forwardRef<
  React.ElementRef<"div">,
  React.ComponentPropsWithoutRef<"div"> & { index: number, secure?: boolean }
>(({ index, secure = true, className, ...props }, ref) => {
  const inputOTPContext = React.useContext(OTPInputContext)
  const { char, hasFakeCaret, isActive } = inputOTPContext.slots[index]
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "relative flex h-12 w-12 items-center justify-center border border-input rounded-md bg-background text-lg font-medium transition-all",
        isActive && "z-10 ring-2 ring-ring ring-offset-background",
        className
      )}
      {...props}
